...
digits_array = np.concatenate((data.T,labels.reshape(labels.shape[0],-1)),axis=1)
digits_df = df = pd.DataFrame(data=digits_array)
digits_df.columns=["x"+str(i) for i in range(1, 786)]
digits_df = digits_df.rename({'x785': 'y'}, axis=1)
digits_df["y"] = np.where(digits_df["y"]==2,0,1)


X = digits_df.loc[:, digits_df.columns != 'y']
y = digits_df["y"]
